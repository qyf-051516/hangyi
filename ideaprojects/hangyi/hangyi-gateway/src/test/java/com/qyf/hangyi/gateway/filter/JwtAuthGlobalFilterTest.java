...
class JwtAuthGlobalFilterTest {
    ...
    @Test
    void permitAll_assistantInternalPath_shouldPassWithoutJwt() {
        ServerWebExchange exchange = createExchange("/api/assistant/internal/chat", null);
        filter.filter(exchange, chain).block();
        verify(chain).filter(exchange);
    }

    @Test
    void assistantInternalPrefixSpoof_shouldReturn401() {
        ServerHttpResponse response = mock(ServerHttpResponse.class);
        when(response.setComplete()).thenReturn(Mono.empty());
        ServerWebExchange exchange = createExchange(
                "/api/assistant/internal-malicious/chat",
                null,
                response
        );
        filter.filter(exchange, chain).block();
    ...
    void validToken_shouldPass() {
        ...
        when(reqBuilder.headers(any(java.util.function.Consumer.class))).thenReturn(reqBuilder);
    ...
    @Test
    void whitelistPrefixSpoof_shouldReturn401() {
        ServerHttpResponse response = mock(ServerHttpResponse.class);
        when(response.setComplete()).thenReturn(Mono.empty());
        ServerWebExchange exchange = createExchange("/api/auth/login-malicious", null, response);
        filter.filter(exchange, chain).block();
        verify(response).setStatusCode(HttpStatus.UNAUTHORIZED);
    }

    @Test
    void exportQueryToken_shouldPass() {
        when(jwtUtil.isTokenValid("export-token")).thenReturn(true);
        ServerWebExchange exchange = createExchange("/api/schedules/export?token=export-token", null);
        filter.filter(exchange, chain).block();
        verify(jwtUtil).isTokenValid("export-token");
        verify(chain).filter(exchange);
    }

    @Test
    void nonBearerAuthorization_shouldReturn401() {
        ServerHttpResponse response = mock(ServerHttpResponse.class);
        when(response.setComplete()).thenReturn(Mono.empty());
        ServerWebExchange exchange = createExchange("/api/test", "Basic abc", response);
        filter.filter(exchange, chain).block();
        verify(response).setStatusCode(HttpStatus.UNAUTHORIZED);
    }

    @Test
    void filterOrderRunsBeforeNormalFilters() {
        org.assertj.core.api.Assertions.assertThat(filter.getOrder()).isEqualTo(-100);
    }
    ...
    private ServerWebExchange createExchange(String path, String authHeader, ServerHttpResponse response) {
        ...
        if (query != null) {
            for (String pair : query.split("&")) {
                ...
                queryParams.add(parts[0], parts.length > 1 ? parts[1] : "");
            }
        }
