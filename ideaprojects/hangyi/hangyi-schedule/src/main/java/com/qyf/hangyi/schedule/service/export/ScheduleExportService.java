...
public class ScheduleExportService {
    ...
    public void exportScheduleExcel(Long scheduleId, HttpServletResponse response) throws IOException {
        ...
        if (schedule == null) {
            throw new com.qyf.hangyi.common.exception.BusinessException(404, "排班不存在");
        }
        ...
        // Fallback: if no details by scheduleId, try date range from the schedule
        if (details.isEmpty() && schedule.getStartDate() != null && schedule.getEndDate() != null) {
            details = detailMapper.selectList(
                    new LambdaQueryWrapper<ScheduleDetail>()
                            .ge(ScheduleDetail::getWorkDate, schedule.getStartDate())
                            .le(ScheduleDetail::getWorkDate, schedule.getEndDate()));
        }

        Map<Long, String> empNameMap = loadEmployeeNameMapSafely();
        ...
        for (Map.Entry<Long, List<ScheduleDetail>> entry : byEmployee.entrySet()) {
            ...
            row.setEmpNo(empNameMap.getOrDefault(entry.getKey(), String.valueOf(entry.getKey())));

            List<String> shiftList = new ArrayList<>();
            for (LocalDate date : dates) {
                ScheduleDetail d = dateMap.get(date);
                shiftList.add(d != null ? shiftNameMap.getOrDefault(d.getShiftId(), "") : "");
            }
            row.setShifts(String.join(" / ", shiftList));
    ...
    private Map<Long, String> loadEmployeeNameMapSafely() {
        try {
            return loadEmployeeNameMap();
        } catch (Exception e) {
            return Map.of();
        }
    }
